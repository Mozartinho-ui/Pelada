...
class TelaTimes(Screen):
    ...
    def confirm_excluir(self, time_idx, player_idx):
        """
        Abre popup pedindo confirmação para excluir o jogador.
        time_idx: índice do time na lista self.times
        player_idx: índice do jogador dentro do time['jogadores']
        """
        # checagens básicas
        if time_idx < 0 or time_idx >= len(self.times):
            return
        if player_idx < 0 or player_idx >= len(self.times[time_idx]['jogadores']):
            return

        jogador_nome = self.times[time_idx]['jogadores'][player_idx]

        layout = BoxLayout(orientation="vertical", spacing=10, padding=10)
        layout.add_widget(Label(text=f"Excluir jogador: {jogador_nome}?"))
        ...
        buttons.add_widget(btn_excluir)
        layout.add_widget(buttons)

        popup = Popup(title="Confirmar Exclusão", content=layout, size_hint=(0.7, 0.4))

        def cancelar(_):
            popup.dismiss()

        def excluir(_):
            # remove o jogador e atualiza a tela
            try:
                self.times[time_idx]['jogadores'].pop(player_idx)
            except Exception:
                pass
            popup.dismiss()
            self.atualizar_tela()
